...
public:
    AllowMissingFrequency (WordList *wl, const char *frqtype) {
        try {
            src = wl->get_stat(frqtype);
        } catch (FileAccessError&) {}
    }
    double freq(int id) {return src ? src->freq(id) : -1;}
    ~AllowMissingFrequency() {delete src;}
};
            ...
            : curr (0), totalcount(0), totalfreq1(0), totalfreq2(0)
{
    ...
    vector<Frequency*> addfreqs1, addfreqs2;
    for (auto it = addfreqs.begin(); it != addfreqs.end(); it++) {
        addfreqs1.push_back(new AllowMissingFrequency(wl1, (*it).c_str()));
        addfreqs2.push_back(new AllowMissingFrequency(wl2, (*it).c_str()));
    }
    ...
    while (!it->end()) {
        ...
        double *freqs = new double[2*addfreqs.size() + 4 + 2];
