...
#elif defined USE_PCRE2
    #define REGPAT_OPTS REG_NOSUB
    #define REGPAT_ICASE ( PCRE2_CASELESS | REG_NOSUB )
    #define REGPAT_UTF8 ( PCRE2_UTF | PCRE2_UCP )
#elif defined USE_ICU
    #define REGPAT_OPTS 0
...
#include "buildin_tables.cc"
    ...
    ~pcre_locale_tables() {
        for (const auto& l : loc2tab)
            pcre_free((void*) l.second);
    }
...
{
#if defined USE_PCRE || defined USE_ICU || defined USE_PCRE2
...
bool regexp_pattern::compile()
{
    ...
    int errcode = 0;
    size_t erroffset = 0;
    comp = pcre2_compile ((PCRE2_SPTR8) pat, strlen(pat), options, &errcode,
            &erroffset, 0);
    if (!comp) return true;
    // const char *errptr = pcre2_get_error_message (errcode);
    md = pcre2_match_data_create_from_pattern (comp, NULL);
    if (!md) {
        pcre2_code_free (comp);
        comp = 0;
        return true;
    }
    return false;
...
regexp_pattern::~regexp_pattern()
{
    ...
    if (conv)
        ucnv_close (conv);
#endif
#if defined USE_PCRE || USE_PCRE2 || defined USE_REGEX
    delete[] pat;
#else
    free (pat);
#endif
...
#elif defined USE_PCRE2
        return pcre2_match (comp, (PCRE2_SPTR8) str, strlen (str), 0, 0, md, 0) >= 0;
#elif defined USE_ICU
        UErrorCode err = U_ZERO_ERROR;
