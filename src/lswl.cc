...
int main (int argc, char **argv)
{
    ...
    int o;
    int ret = 0;
    while ((o = getopt (argc, argv, "l:r:i:s:a:b:w:f:x::")) != -1) {
        switch (o) {
        case 'l':
            items = atoll (optarg);
            break;
    ...
    Corpus *c = NULL, *oc = NULL;
    vector<Frequency*> addfreqs;
    Frequency *sortfreq = NULL;
    WordList *l = NULL;
    bool free_l = false;
#ifdef SKETCH_ENGINE
    NGram *ngr = NULL;
    ...
    try {
        ...
        if (!strcmp(wl, "WSCOLLOC")) {
            ...
            ngr = new NGram (c->get_conf("PATH") + "/" + wl, fpath);
            l = ngr->get_wordlist ();
            free_l = true;
        } else
    #endif
        ...
        sortfreq = l->get_stat(sorttype);
        wordlist (l, wlpat.c_str(), addfreqs, sortfreq, wlist, blist, minfreq, maxfreq, items, nonwordre, out_list, out_count, out_count_total);
