...
void write_tokenfile(Corpus *corp, SubCorpus *subc, string &structname, vector<string> &attrnames, string &base) {
    ...
    for (size_t i = 0; i < attrs.size(); i++) {
        ...
        if (struc->conf->str2bool(ao["MULTIVALUE"])) {
            ...
            std::string multisep = ao["MULTISEP"];
            if (multisep.length())
                multisep = multisep.substr(0, 1);
            update_multivalue(attrs[i], tokencov[i], multisep.c_str());
        }
    }
...
int main (int argc, char **argv) {
    ...
    try {
        ...
        if (!structname.empty() && !attrname.empty()) {
            ...
            stringstream ss (corp->get_struct(string(structname))->get_conf("ATTRLIST"));
